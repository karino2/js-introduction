...
今回はプラグインのコードを読んで行きつつ、これまで説明が漏れていた事について補足していったり、読まなくて良い部分はどう乗り切るのか、
みたいな事を参考にしてもらえたらな、と思います。
...
JavaScriptの大規模なプログラムでは、このように関数を辞書に入れた物をいろいろ集めて最終的なゲームなどを作ります。
そしてプラグインというのは、この特定の部分の辞書を上書きして自分の処理に差し替える、というのが基本となります。
...
は全く同じ意味です。違いが気にならないなら気にしなくてもOK。なお、自分で書く時はこれまでやった通り、いつも下のように書きましょう。（理由は秘密、私を信じて！）
