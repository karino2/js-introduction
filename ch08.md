...
の二種類に変更しましょう。
ダメージは斧の時に20, 棍棒の時に5のままでOKです。
二箇所書き換える場所があるので注意しましょう。

<script>
yesNoAlertQuestionWithLabelsPush("q11", "何で攻撃する？", "斧", "棍棒", "斧で攻撃、20のダメージ", "斧を選んだのに「斧で攻撃、20のダメージ」と表示されない。",
 "棍棒で攻撃、5のダメージ", "棍棒で攻撃したのに「棍棒で攻撃、5のダメージ」と表示されない。");
</script>
...
10回に1回の割合でクリティカルの出るコードがあります。
このコードを読んで理解し、クリティカルの割合を2回に1回出るように改造しましょう。
...
qobj.scenarios.push({
    ...
    verify: (intp) => {
        ...
        if (counts["クリティカル！30のダメージ"] < 80) {
            return "クリティカルが出なさすぎ";
        }
        if(counts["クリティカル！30のダメージ"] > 120) {
            return "クリティカルが出すぎ";
        }
        return true;  
    }
...
var critial = makeCritical();
if (critial == 1) {
    MessageBox.show("クリティカル！30のダメージ");
} else {
    MessageBox.show("10のダメージ");
}
</textarea>
    <b>結果:</b> <span class="console"></span><br>
    <span class="result"></span><br>
...
答え:<br>
var makeDamage = function() {<br>
&nbsp;&nbsp;&nbsp;&nbsp;var saikoro = Math.randomInt(10);<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;if(saikoro &lt; 5) {<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return 1;<br>
&nbsp;&nbsp;&nbsp;&nbsp;} else {<br>
