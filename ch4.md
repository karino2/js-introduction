...
それは、上記のような提供されている機能の仕様を順番に調べていく、という作業になる事でしょう。
...
JavaScriptでは、変数が1かどうかは、`変数==1`でチェックします。
なお、0かどうかは`変数==0`でチェックします。
`==`とイコールを2つ並べるんですね。  
うわ、なんか知らない奴だ！難しそう！と思うかもしれませんが、ただそう決まっている、というだけです。

JavaScriptにおいては、普通の`=`は、「右の結果を左の箱に代入する」という意味で使われているので、左と右が同じ、という意味では使えません。（以前少しコラムでも触れました）  
そこでJavaScriptでは`==`が、左と右が同じ、という意味にしよう、とNetscape社の人が昔決めたのです。
...
今回は、解説しているif自体はそんな難しくないと思うのですが、課題は全体的に難易度が高い気がします。
...
　  
このシリーズは、いろんな事を説明するよりは、良く使う少数の物だけを詳しく解説していく、という方針で書いています。
