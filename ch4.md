...
<div id="q3">
    ...
    // このままでは6の時も実行されてしまうので、5の時だけ実行されるように以下を変更
    MessagebBox.show("sentakuは5です。");
...
`sentaku`は、プレーヤーが「はい」を選んだら`1`に、「いいえ」を選んだら`0`になる、という話でしたから、
これは「プレーヤーが「はい」を選んだら」、という意味になります。
