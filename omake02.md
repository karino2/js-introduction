...
このプラグインは、基本的にはこのコードの最後のifの所で、以下のようになっているのを、
...
このコード以外の所は何か分からなくても、ここが何かさえわかれば良い。

そしてifの所の条件が`if (result.critical) {`となっていて、実行されるのが`this.setupCriticalEffect();`と書いているのだから、
なんとなく文字からだけでもクリティカルの時にクリティカル用のエフェクトを出している場所なんだな、という気がします。

そしてそこに、このプラグインでは新たに`target.startAnimation(37, false, 0);`という物を追加する。
...
なんかこれは普通のツクールの条件分岐とかっぽく無いですか？（良く知らないけど）
...
で、関数の一つ目の0と２で、「どんな数字を画面に出すか」を指定するっぽい。
0でHPダメージ、2でMPダメージらしい。
じゃあ1はなんだろう？とか思ってしまう。回復？分からん。まぁどうでもいい。
...
`} else if (result.hpAffected) { `という事で、
...
で、aliveというのは生きている、という意味です。isAliveで生きている。

つまり、「モンスターがまだ生きている」というのと、「result.mpDamageがゼロ以外の値」の両方だったら、という意味になります。
...
効果0とか効果2がどういう表示かは自分は知らないですが、読者のみなさんは知ってる事でしょう。


### 本題の所も一応読む

ここまで分かると肝心のプラグインのコードはむしろ説明は要らないのでは？最初にちらっと解説しましたが、一応あらためて見ておきます。
以下の最後のif文ですね。
...
`target.startAnimation(37, false, 0);`という行を追加する、という奴ですね。`this`はふわっと出る文字、`target`は攻撃が当たるモンスター。
数字を細工したいなら`this`、モンスター側を揺らしたり色変えたりしたければ`target`。

で、モンスター側に雷を落とすという事で以下のようなコードに変更している。
...
これはたぶん、プラグインのパラメータは全部文字なんでしょう。

だから、例えば使う人が37と数字を指定しても、`"37"`という、偶然数字に見えるけど文字が入ってる、と判断されちゃうみたいです。
で、この文字から数字にする為に、この人はNumberという関数を使っている、という事です。

このシリーズではあえて文字と数字の区別とかはあんまり真面目に解説してないので、この辺は最初の頃はそのままやっていって、
なんか数字のはずなのにうまく動かなかったらNumberつける、とかその位でいいかもしれません。
