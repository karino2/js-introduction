...
| 元の単語 | 置き換える単語 | 解説 |
| ------- | ---------- | ------- |
| Game_Interpreter | ゲーム主 | インタープリタというのがどういう意味で使っているのか分からないですが、ルールとかスクリプトを解釈して実行する物、という意味だと思います。 |
...
辞書の中に辞書が入っている、という事で一見なかなか複雑です。100本ノックでこの位ならやりましたが、もうまぁまぁ忘れてもいる事でしょう。
ですが、prototypeはちょっと特殊な奴なので、実はそんな大変じゃない。

prototypeの詳細は最後にちょろっと解説しますが、現時点では「その前の辞書名とprototypeを合わせて全体で一つの辞書の名前」だと思っておいてOKな物です。

つまりこの場合、`ゲーム主.prototype`全部でツクールMVが提供している辞書だと思っておきましょう。
...
つまり以上のコードで、`ゲーム主.prototype`という辞書の中の`pluginCommand`というキーで中身を取り出しています。
辞書のキーは[第六回](ch06.md)を参考にしてください。

現時点での「ゲーム主.ptotoype」の中身をテーブルで書いておきましょう。
...
プラグインのコードを読む練習として、元の英語に戻したコードを見てみましょう。
...
上の置き換えた版と何度も見比べてみて下さい。分かりそうですか？
以上でプラグインの解説は終わりです。
