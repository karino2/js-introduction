...
変数と同様、まず言葉が難しいですよね。関数。しかもこれ、数学の言葉じゃない？中学生くらいでやる気がする。

数学って、算数で挫折したって言ってんだろー！って話になりますが、
算数の関数とプログラムの関数は、似ている所もありますが、結構違います。
なので数学の関数は分からなくても問題無い。

ただ、数学の関数分からなくても問題無いけど、簡単では無いです。
...
これらの実際の話に入る前に、この難しさについて軽く触れておく事で、
...
関数を作って、その出来た関数を別の所で使う、というのが関数の基本となります。
...
使う所はこれまでと同じ話なので大した事無いんですが、作る所はちょっとそれ以外の所と違う事がある。
...
### 関数を作る人と使う人が別の人な場合がある

関数は作る場所と使う場所が別なので、
たまに作る人と使う人が別な事があります。

例えばUpdateという関数は作るのは我々だけど使うのはゲームエンジン、という事があります。
この時、我々は使う人の事は良く知らない場合があります。

この、使う人の事は知らないけど、使われる側の人として関数を作る、
というのは、「使われる」という「られる」側の立場で考えないといけなくて、
これがなかなか難しい。

まぁ難しい難しい言ってても始まらないので、実際に見ていきましょう。
