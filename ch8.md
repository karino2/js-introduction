...
このコードは、関数を作る側では「0から11の数字のうち、最初の5個はそれぞれ1から5までの目として、後ろの7個は全部6とする」と理解します。

で、ひとたび完成したら、`使う`側ではその事実は忘れて、ぼんやりした概要、
この場合は「1から6の目を返すサイコロだけど、6が多め」という物だと考えます。

この時、それがどう実現されているかは「忘れる」のがポイントです。
あくまで１から6を返すサイコロだけど、



`return`というのは、関数を`作る`側と、`使う`側の間でやりとりする為のものです。
