...
そこで実際にどう使われるかは、課題をいろいろやって見ていきましょう。
...
さらにawa.cheat()が実行されたらtoot数が100増えるようにせよ。
...
と表示するようにせよ。見た目より難しい問題です。(書く量は短いけど)
...
　  
最後に配列とかも混ぜ、割と良くある感じの課題をやって終わりにしましょう。
...
　  
こういうコードはいかにも実際にある奴なので、課題としては難しいですね。
...
　  
あなたはJavaScriptの入門としては十分過ぎる所まで来ていると思います。
実際、第十回の内容は中級でやる事でしょう。関数もarguments自体は中級の内容になるので、第九回も半分くらいは中級です。
本屋さんでJavaScriptの本を立ち読みしたら、入門の本には書いて無いと思います。試しに本屋に行ったら見てみてください。  
　  
ここ何回かの課題を見れば分かるように、ここまでやった内容は一つ一つは小さな事でも、
全部組み合わせると相当いろいろな事が書けます。  
　  
個々の要素が分かる、という事と組み合わせて作りたい物を作る事はまた違う難しさもありますが、
組み合わせて作る練習はプログラムの勉強の次のステップで、実際に作りながら学んでいく物です。
勉強の段階では個々の要素が分かる所までで十分です。
勉強するだけで作れるようにならないのはどこの世界でも一緒ですよね。
なので勉強としてはその手前までで十分マスターした、と言って良いです。  
　  
書き始めるまでが勉強の役割、書いていく内に分かっていく事は勉強の段階では分からなくてもOKなのです。  
　  
さて、第十一回はこれまでやった事と全く同じ事を違う方法でも書ける、という話になります。
出来る事は一切増えないのですが、人のコードを読む時などに知らない書き方をされていると戸惑う事もあるので、
良くある書き方は一応見ておこうと思います。
ただ戸惑う程度なので知らなくてもどうという事はありません。  
　  
